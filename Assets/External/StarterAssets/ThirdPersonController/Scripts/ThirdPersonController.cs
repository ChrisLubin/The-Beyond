...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : NetworkBehaviour
    {
        ...
        private void CameraRotation()
        {
            ...
            if (InputSystem.look.sqrMagnitude >= _threshold && !LockCameraPosition)
            {
                //Don't multiply mouse input by Time.deltaTime;
                float deltaTimeMultiplier = IsCurrentDeviceMouse ? 1.0f : Time.deltaTime;

                _cinemachineTargetYaw += InputSystem.look.x * deltaTimeMultiplier;
                _cinemachineTargetPitch += InputSystem.look.y * deltaTimeMultiplier;
            }
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                // Jump
                if (InputSystem.jump && _jumpTimeoutDelta <= 0.0f)
                {
